...
class PerformanceAnalytics:
    ...
    @staticmethod
    def calculate_metrics(history: List[PortfolioSnapshot], initial_capital: float) -> Dict:
        if not history:
            return {}
        ...
        sharpe_ratio = (df["returns"].mean() / volatility * np.sqrt(len(df))) if volatility > 0 else 0
